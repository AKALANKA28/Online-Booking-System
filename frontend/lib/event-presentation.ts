...
const THEMES: EventTheme[] = [
  {
    name: "Cobalt Pulse",
    background:
      "linear-gradient(135deg, #0F1732 0%, #1A2B5F 42%, #2858FF 100%)",
    accent: "#2459FF",
    subtle: "#E7ECFF",
  ...
  },
  {
    name: "Ember Room",
    background:
      "linear-gradient(135deg, #221311 0%, #61241C 48%, #E86C4D 100%)",
    accent: "#E86C4D",
    subtle: "#FDE7E1",
  ...
  },
  {
    name: "Forest House",
    background:
      "linear-gradient(135deg, #0E1E1A 0%, #144D3B 48%, #1E7855 100%)",
    accent: "#1E7855",
    subtle: "#DEF4EA",
...
const CATEGORY_MAP: Array<{ label: string; matchers: string[] }> = [
  {
    label: "Concert",
    matchers: ["music", "live", "concert", "festival", "dj", "tour"],
  },
  {
    label: "Conference",
    matchers: ["summit", "expo", "conference", "tech", "design", "forum"],
  },
  { label: "Cinema", matchers: ["cinema", "film", "movie", "screening"] },
  {
    label: "Theatre",
    matchers: ["theatre", "theater", "play", "musical", "stage"],
  },
  {
    label: "Experience",
    matchers: ["immersive", "dining", "experience", "popup"],
  },
];
...
export function getCategory(event: EventRecord) {
  ...
  const match = CATEGORY_MAP.find((entry) =>
    entry.matchers.some((keyword) => haystack.includes(keyword)),
  );
...
export function availabilitySnapshot(seats: SeatRecord[]) {
  ...
  const cancelled = seats.filter((seat) => seat.status === "CANCELLED").length;
  return {
    available,
    reserved,
    booked,
    ...
    total: seats.length,
    sellThrough:
      seats.length === 0 ? 0 : Math.round((booked / seats.length) * 100),
  };
}
